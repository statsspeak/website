...
- **Density (Form C).** A single `<rect>` driven by one `<filter>` pipeline. The filter chain is exactly `feTurbulence → feColorMatrix → feGaussianBlur`. No additional layers, no second cloud, no Perlin overlays. The rect is oversized (≥ ±15 beyond viewBox) and wrapped in an inner `<g>` that drifts in SVG-user space; the rect itself is never resized at runtime.
...
- **Drift.** Mesh rotation accumulators ≤ 0.001 rad / frame (Form A). Per-node sine amplitude ≤ 0.025 world units, phase multiplier ≤ 0.25 (Form B). Form C combines two drift channels: (1) `baseFrequency` swing of ±40–50% of the base value across a cycle ≥ 20 seconds, and (2) spatial translation of the inner group up to ±8% of the viewBox across a cycle ≥ 30 seconds. The motion should be unmistakably *there* — the previous "≤ 25% of base" guidance made it mathematically real but visually invisible, and was retired in revision N+3 after a "I can't see the motion" report. Premium quiet, not premium absent.
...
- **Scrim.** A CSS overlay (`statsspeak-hero-scrim`) fades the canvas into `--bone` under the headline column (left side ≥ 90% bone) and lets it breathe in the right column (≤ 10% bone at the far right). Form C also carries an internal SVG `<mask>` doing the same fade. The mask is anchored to **viewBox space**, not to the drifting inner group — so as the cloud translates, the right-side weighting stays put.
...
_Last revised: 2026-05-28 — added §6.7 (hero canvas carve-out), §9.5 (brand-mark casing), §15 (premium positioning audit), CTA sentence-case rule; revised §9.1 headline guidance and §13.2 P5 hero migration item to reflect the canvas direction; raised §6.7 visibility caps after the hero canvas read as invisible against the scrim; iterated §6.7 across three canvas forms (Form C ethereal SVG shadow — current default; Form A noise-displaced wireframe icosahedron — documented alternate; Form B sparse constellation — documented fallback) after evaluating two external generative-art hero components; **relaxed §6.7 Form C drift caps after the first visibly-animated revision was still imperceptible** — feature scale must now produce 2–4 visible cloud cells across the viewBox, `baseFrequency` swing ±40–50%, and an additional spatial translation channel up to ±8% of the viewBox. This document supersedes every prior styling decision in the codebase. Where this document and the code disagree, the document is correct and the code is a bug._
